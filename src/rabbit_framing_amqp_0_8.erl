...
-type(amqp_method_field_name() ::
       ( operation | heartbeat | arguments | consume_rate
       | requeue | queue | message_count | exchange
       | consumer_count | staged_size | no_local | routing_key
       | delivery_tag | exclusive | queue | queue
       | ticket | result | queue | mandatory
       | exchange | no_local | consumer_tag | routing_key
       | nowait | channel_max | arguments | exchange
       | nowait | nowait | reply_text | routing_key
       | prefetch_count | ticket | version_minor | ticket
       | queue | string_2 | nowait | nowait
       | ticket | mandatory | exchange | message_count
       | mandatory | exclusive | cluster_id | server_properties
       | routing_key | string_op | ticket | queue
       | if_unused | global | if_empty | delivery_tag
       | nowait | mechanisms | requeue | prefetch_count
       | delivery_tag | string_result | global | content_size
       | message_count | consumer_tag | consumer_tag | delivery_tag
       | ticket | virtual_host | locales | string_1
       | delivery_tag | exchange | routing_key | arguments
       | consumer_tag | no_ack | reply_code | exclusive
       | reply_text | nowait | capabilities | identifier
       | reply_code | consumer_tag | delivery_tag | prefetch_size
       | requeue | nowait | client_properties | global
       | multiple | queue | consumer_tag | integer_3
       | routing_key | ticket | ticket | prefetch_size
       | multiple | queue | requeue | consumer_tag
       | reply_code | exchange | no_local | no_ack
       | no_ack | exchange | table | out_of_band
       | ticket | nowait | active | consumer_tag
       | redelivered | active | prefetch_size | consumer_tag
       | prefetch_count | reply_code | integer_2 | immediate
       | reply_text | details | dtx_identifier | identifier
       | reply_code | nowait | host | reply_text
       | delivery_tag | class_id | method_id | exchange
       | integer_result | routing_key | immediate | version_major
       | ticket | realm | reply_text | known_hosts
       | exclusive | passive | redelivered | active
       | write | read | consumer_tag | queue
       | mechanism | redelivered | response | identifier
       | consumer_tag | locale | integer_4 | immediate
       | consumer_tag | ticket | challenge | exchange
       | response | type | durable | queue
       | passive | channel_max | result | frame_max
       | internal | nowait | reply_text | arguments
       | routing_key | exchange | frame_max | content_checksum
       | heartbeat | exchange | ticket | exchange
       | if_unused | durable | insist | meta_data
       | class_id | message_count | known_hosts | routing_key
       | exchange | operation | routing_key | exchange
       | ticket | consumer_tag | consumer_tag | integer_1
       | queue | ticket | consumer_tag | method_id
       | passive | auto_delete | routing_key | reply_code
       | exclusive | delivery_tag | auto_delete | integer_op
       | nowait | ticket )).

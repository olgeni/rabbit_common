...
-type(amqp_method_field_name() ::
       ( integer_2 | exchange | internal | nowait
       | string_1 | ticket | arguments | string_2
       | operation | ticket | delivery_tag | exchange
       | exchange | exchange | routing_key | if_unused
       | no_local | nowait | consumer_tag | message_count
       | requeue | table | dtx_identifier | integer_op
       | queue | mandatory | ticket | delivery_tag
       | queue | exchange | content_checksum | passive
       | exchange | durable | integer_1 | exclusive
       | nowait | auto_delete | no_ack | queue
       | arguments | nowait | realm | arguments
       | requeue | queue | identifier | operation
       | message_count | consumer_count | no_ack | integer_result
       | exclusive | exclusive | exclusive | consumer_tag
       | ticket | queue | content_size | staged_size
       | exchange | exchange | routing_key | requeue
       | exclusive | nowait | mechanisms | arguments
       | requeue | routing_key | nowait | result
       | multiple | delivery_tag | locales | ticket
       | queue | queue | nowait | ticket
       | routing_key | exchange | mandatory | routing_key
       | message_count | consumer_tag | redelivered | mandatory
       | reply_text | immediate | consume_rate | string_op
       | ticket | queue | if_unused | if_empty
       | exchange | read | ticket | nowait
       | queue | prefetch_count | queue | global
       | routing_key | message_count | consumer_tag | delivery_tag
       | ticket | routing_key | ticket | routing_key
       | queue | consumer_tag | heartbeat | routing_key
       | channel_max | delivery_tag | response | frame_max
       | string_result | heartbeat | nowait | consumer_tag
       | virtual_host | capabilities | insist | consumer_tag
       | identifier | prefetch_size | prefetch_size | cluster_id
       | known_hosts | reply_code | prefetch_count | delivery_tag
       | host | known_hosts | consumer_tag | delivery_tag
       | reply_code | nowait | reply_text | multiple
       | class_id | method_id | integer_3 | consumer_tag
       | consumer_tag | global | no_local | immediate
       | global | challenge | out_of_band | nowait
       | exchange | nowait | active | result
       | reply_code | consumer_tag | prefetch_size | redelivered
       | active | consumer_tag | exchange | reply_code
       | redelivered | reply_text | details | identifier
       | consumer_tag | reply_code | response | reply_text
       | class_id | integer_4 | method_id | exchange
       | reply_text | routing_key | delivery_tag | routing_key
       | version_major | version_minor | server_properties | passive
       | prefetch_count | no_ack | active | write
       | reply_code | client_properties | channel_max | mechanism
       | ticket | ticket | locale | consumer_tag
       | reply_text | ticket | ticket | frame_max
       | exchange | ticket | consumer_tag | type
       | immediate | no_local | passive | meta_data
       | durable | auto_delete )).

...
-type(amqp_method_field_name() ::
       ( queue | string_1 | passive | staged_size
       | durable | result | auto_delete | queue
       | meta_data | internal | nowait | arguments
       | multiple | consumer_tag | routing_key | string_2
       | reply_code | exchange | operation | routing_key
       | ticket | exchange | nowait | exchange
       | if_unused | nowait | ticket | ticket
       | routing_key | ticket | table | mandatory
       | result | integer_op | ticket | queue
       | consume_rate | passive | queue | queue
       | durable | exchange | exclusive | delivery_tag
       | auto_delete | ticket | nowait | consumer_tag
       | integer_2 | arguments | queue | requeue
       | exchange | string_op | routing_key | queue
       | message_count | content_checksum | delivery_tag | consumer_count
       | integer_result | no_local | content_size | exclusive
       | ticket | queue | queue | mandatory
       | exchange | routing_key | delivery_tag | routing_key
       | nowait | arguments | consumer_tag | arguments
       | version_minor | ticket | identifier | ticket
       | cluster_id | queue | integer_1 | reply_code
       | nowait | exclusive | nowait | no_ack
       | ticket | server_properties | exchange | message_count
       | multiple | immediate | operation | ticket
       | exclusive | passive | queue | integer_3
       | if_unused | global | if_empty | nowait
       | delivery_tag | prefetch_count | global | string_result
       | delivery_tag | message_count | active | consumer_tag
       | consumer_tag | delivery_tag | channel_max | frame_max
       | redelivered | reply_text | heartbeat | routing_key
       | channel_max | reply_code | frame_max | heartbeat
       | nowait | requeue | redelivered | virtual_host
       | client_properties | message_count | capabilities | insist
       | known_hosts | consumer_tag | prefetch_count | routing_key
       | routing_key | host | ticket | known_hosts
       | exchange | reply_text | reply_code | reply_text
       | prefetch_size | class_id | consumer_tag | method_id
       | requeue | response | consumer_tag | dtx_identifier
       | reply_text | no_local | consumer_tag | no_ack
       | no_ack | consumer_tag | out_of_band | nowait
       | immediate | nowait | prefetch_size | integer_4
       | prefetch_size | consumer_tag | active | consumer_tag
       | no_local | active | exchange | reply_code
       | nowait | reply_text | consumer_tag | details
       | identifier | exchange | reply_code | reply_text
       | class_id | global | challenge | method_id
       | immediate | exchange | routing_key | mandatory
       | version_major | realm | routing_key | exchange
       | identifier | exclusive | exchange | mechanisms
       | delivery_tag | redelivered | locales | write
       | read | requeue | mechanism | ticket
       | prefetch_count | response | locale | consumer_tag
       | delivery_tag | consumer_tag | ticket | ticket
       | exchange | type )).

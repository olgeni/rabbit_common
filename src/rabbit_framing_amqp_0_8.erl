...
-type(amqp_method_field_name() ::
       ( routing_key | read | insist | requeue
       | known_hosts | prefetch_count | host | dtx_identifier
       | known_hosts | ticket | routing_key | routing_key
       | reply_code | reply_text | class_id | method_id
       | operation | consumer_tag | identifier | ticket
       | string_result | no_local | ticket | no_ack
       | integer_4 | out_of_band | reply_code | nowait
       | no_ack | active | consumer_tag | reply_text
       | prefetch_size | consumer_tag | prefetch_size | active
       | consumer_tag | exchange | immediate | reply_code
       | reply_text | details | consumer_tag | delivery_tag
       | nowait | reply_code | reply_text | class_id
       | method_id | integer_1 | exchange | immediate
       | global | routing_key | challenge | version_major
       | no_local | routing_key | version_minor | server_properties
       | consumer_tag | mechanisms | result | locales
       | prefetch_size | consumer_tag | redelivered | write
       | client_properties | mechanism | exchange | response
       | mandatory | exchange | queue | locale
       | consumer_tag | consumer_tag | prefetch_count | ticket
       | response | exchange | type | passive
       | ticket | consumer_tag | channel_max | operation
       | auto_delete | delivery_tag | queue | routing_key
       | meta_data | internal | integer_2 | exclusive
       | nowait | arguments | consumer_tag | frame_max
       | string_op | string_1 | heartbeat | staged_size
       | ticket | exchange | multiple | exchange
       | durable | if_unused | requeue | nowait
       | identifier | message_count | table | ticket
       | frame_max | integer_op | ticket | nowait
       | ticket | routing_key | queue | content_checksum
       | passive | durable | exclusive | exchange
       | auto_delete | heartbeat | nowait | exchange
       | nowait | queue | arguments | consume_rate
       | requeue | delivery_tag | queue | message_count
       | delivery_tag | queue | consumer_count | routing_key
       | content_size | integer_result | no_local | exclusive
       | consumer_tag | ticket | queue | exchange
       | mandatory | routing_key | channel_max | nowait
       | arguments | reply_text | consumer_tag | consumer_tag
       | nowait | ticket | string_2 | delivery_tag
       | realm | ticket | queue | nowait
       | ticket | exchange | exclusive | routing_key
       | message_count | identifier | mandatory | cluster_id
       | immediate | reply_code | ticket | delivery_tag
       | exchange | queue | if_unused | requeue
       | delivery_tag | if_empty | passive | multiple
       | nowait | prefetch_count | queue | global
       | message_count | integer_3 | consumer_tag | active
       | delivery_tag | virtual_host | result | ticket
       | global | queue | exchange | exchange
       | routing_key | arguments | reply_code | capabilities
       | no_ack | reply_text | exclusive | nowait

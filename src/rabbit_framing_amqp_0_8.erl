...
-type(amqp_method_field_name() ::
       ( staged_size | routing_key | dtx_identifier | channel_max
       | queue | meta_data | frame_max | heartbeat
       | nowait | consumer_tag | virtual_host | queue
       | capabilities | ticket | insist | exchange
       | nowait | routing_key | ticket | known_hosts
       | prefetch_count | host | arguments | immediate
       | known_hosts | mandatory | result | reply_code
       | reply_text | consumer_tag | class_id | method_id
       | exclusive | no_ack | consumer_tag | server_properties
       | no_local | no_ack | consumer_tag | identifier
       | out_of_band | nowait | exclusive | delivery_tag
       | active | delivery_tag | consumer_tag | integer_result
       | active | content_size | operation | exchange
       | reply_code | reply_text | no_local | delivery_tag
       | details | reply_code | multiple | identifier
       | requeue | reply_code | reply_text | class_id
       | consumer_tag | string_1 | consumer_tag | method_id
       | redelivered | reply_text | routing_key | exchange
       | routing_key | identifier | mandatory | global
       | cluster_id | realm | ticket | reply_code
       | exclusive | nowait | passive | requeue
       | exchange | active | client_properties | integer_4
       | write | read | integer_2 | routing_key
       | string_op | ticket | integer_1 | routing_key
       | if_unused | consumer_tag | global | ticket
       | exchange | requeue | type | consume_rate
       | passive | string_result | durable | auto_delete
       | response | reply_text | internal | nowait
       | arguments | string_2 | prefetch_size | locale
       | operation | consumer_tag | ticket | exchange
       | if_unused | nowait | redelivered | message_count
       | nowait | consumer_tag | delivery_tag | immediate
       | integer_op | ticket | queue | passive
       | durable | global | delivery_tag | exclusive
       | immediate | reply_text | auto_delete | nowait
       | arguments | integer_3 | prefetch_size | requeue
       | ticket | queue | message_count | redelivered
       | prefetch_size | consumer_count | no_ack | consumer_tag
       | exclusive | no_local | ticket | queue
       | prefetch_count | exchange | exchange | consumer_tag
       | routing_key | queue | nowait | routing_key
       | exchange | arguments | result | content_checksum
       | exchange | consumer_tag | ticket | delivery_tag
       | nowait | delivery_tag | routing_key | exchange
       | ticket | queue | nowait | mandatory
       | version_major | multiple | channel_max | version_minor
       | reply_code | exchange | message_count | mechanisms
       | locales | routing_key | table | ticket
       | queue | consumer_tag | ticket | mechanism
       | consumer_tag | frame_max | if_empty | nowait
       | prefetch_count | challenge | ticket | message_count
       | queue | exchange | response | exchange
       | queue | delivery_tag | routing_key | ticket
       | queue | heartbeat )).

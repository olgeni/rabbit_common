...
-type(amqp_method_field_name() ::
       ( requeue | redelivered | mechanisms | exchange
       | locales | exchange | ticket | routing_key
       | queue | exclusive | if_unused | response
       | nowait | delivery_tag | locale | no_local
       | consumer_tag | challenge | immediate | message_count
       | consumer_tag | response | routing_key | reply_code
       | ticket | ticket | string_1 | frame_max
       | heartbeat | routing_key | nowait | channel_max
       | identifier | frame_max | if_unused | reply_text
       | heartbeat | requeue | dtx_identifier | delivery_tag
       | virtual_host | capabilities | consumer_tag | insist
       | message_count | client_properties | ticket | known_hosts
       | prefetch_count | table | routing_key | host
       | integer_4 | known_hosts | queue | prefetch_count
       | routing_key | reply_code | mechanism | reply_text
       | queue | class_id | meta_data | exchange
       | method_id | prefetch_size | consumer_tag | exchange
       | no_local | no_ack | if_empty | exchange
       | mandatory | out_of_band | nowait | nowait
       | consumer_tag | integer_1 | ticket | active
       | string_2 | consumer_tag | active | consumer_tag
       | nowait | exchange | nowait | reply_code
       | content_size | reply_text | consume_rate | details
       | queue | exchange | result | identifier
       | result | reply_code | prefetch_count | immediate
       | reply_text | routing_key | class_id | string_result
       | global | method_id | routing_key | exchange
       | exclusive | content_checksum | routing_key | consumer_tag
       | version_major | prefetch_size | delivery_tag | realm
       | consumer_tag | consumer_tag | exclusive | consumer_tag
       | passive | active | integer_result | write
       | redelivered | consumer_tag | read | operation
       | reply_code | integer_op | ticket | exchange
       | routing_key | nowait | ticket | cluster_id
       | delivery_tag | exchange | delivery_tag | type
       | passive | durable | auto_delete | reply_text
       | internal | delivery_tag | reply_code | nowait
       | multiple | operation | arguments | ticket
       | exchange | ticket | global | channel_max
       | ticket | nowait | integer_2 | delivery_tag
       | exchange | ticket | ticket | queue
       | exchange | queue | queue | passive
       | string_op | durable | reply_text | exclusive
       | auto_delete | nowait | delivery_tag | arguments
       | routing_key | arguments | queue | global
       | message_count | identifier | consumer_count | immediate
       | queue | mandatory | mandatory | requeue
       | ticket | consumer_tag | queue | no_ack
       | exchange | multiple | routing_key | nowait
       | ticket | arguments | consumer_tag | version_minor
       | prefetch_size | redelivered | no_local | requeue
       | ticket | no_ack | server_properties | queue
       | exclusive | nowait | consumer_tag | integer_3
       | staged_size | message_count )).

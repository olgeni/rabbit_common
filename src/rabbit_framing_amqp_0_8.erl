...
-type(amqp_method_field_name() ::
       ( no_local | requeue | type | durable
       | exclusive | auto_delete | nowait | global
       | arguments | no_local | string_op | delivery_tag
       | queue | delivery_tag | exchange | message_count
       | integer_3 | consumer_count | no_local | delivery_tag
       | exclusive | ticket | multiple | durable
       | nowait | queue | exchange | integer_2
       | routing_key | ticket | nowait | arguments
       | identifier | ticket | string_result | ticket
       | consumer_tag | auto_delete | reply_code | string_1
       | ticket | immediate | reply_text | queue
       | nowait | queue | internal | exchange
       | immediate | ticket | message_count | delivery_tag
       | reply_text | reply_code | consumer_tag | ticket
       | routing_key | consumer_tag | queue | queue
       | if_unused | exchange | if_empty | nowait
       | consumer_tag | consumer_tag | delivery_tag | requeue
       | channel_max | message_count | exchange | routing_key
       | exchange | ticket | queue | exchange
       | exchange | version_minor | no_ack | routing_key
       | string_2 | arguments | ticket | no_ack
       | exclusive | ticket | delivery_tag | redelivered
       | exclusive | mandatory | operation | server_properties
       | routing_key | routing_key | message_count | prefetch_size
       | ticket | prefetch_count | queue | result
       | global | mechanisms | nowait | prefetch_size
       | nowait | prefetch_count | consumer_tag | identifier
       | reply_code | content_checksum | consumer_tag | reply_text
       | class_id | virtual_host | method_id | active
       | consumer_tag | integer_1 | exchange | requeue
       | nowait | staged_size | dtx_identifier | out_of_band
       | cluster_id | reply_text | redelivered | capabilities
       | exchange | active | result | consumer_tag
       | active | queue | reply_code | consumer_tag
       | insist | reply_text | read | details
       | routing_key | routing_key | reply_code | mandatory
       | meta_data | reply_text | multiple | class_id
       | routing_key | method_id | reply_code | delivery_tag
       | version_major | realm | queue | delivery_tag
       | exclusive | exchange | consumer_tag | table
       | passive | ticket | locales | write
       | consumer_tag | integer_4 | client_properties | routing_key
       | mechanism | routing_key | exchange | response
       | prefetch_size | content_size | locale | consumer_tag
       | consume_rate | nowait | challenge | exchange
       | response | consumer_tag | passive | nowait
       | host | channel_max | prefetch_count | frame_max
       | heartbeat | requeue | nowait | integer_op
       | arguments | global | identifier | frame_max
       | ticket | heartbeat | known_hosts | ticket
       | consumer_tag | exchange | integer_result | if_unused
       | no_ack | nowait | operation | mandatory
       | known_hosts | redelivered | immediate | ticket
       | queue | passive )).

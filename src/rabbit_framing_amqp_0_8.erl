...
-type(amqp_method_field_name() ::
       ( reply_text | reply_code | ticket | frame_max
       | content_size | heartbeat | routing_key | channel_max
       | frame_max | ticket | channel_max | heartbeat
       | ticket | nowait | delivery_tag | virtual_host
       | routing_key | capabilities | operation | insist
       | consumer_tag | queue | exchange | known_hosts
       | string_op | prefetch_count | global | identifier
       | host | known_hosts | reply_code | delivery_tag
       | nowait | reply_code | delivery_tag | exchange
       | reply_text | routing_key | cluster_id | class_id
       | method_id | consumer_tag | consumer_tag | multiple
       | no_local | no_ack | queue | mandatory
       | out_of_band | consumer_tag | global | nowait
       | active | consumer_tag | string_result | active
       | ticket | arguments | exchange | version_minor
       | reply_code | reply_code | reply_text | multiple
       | details | identifier | reply_code | no_ack
       | server_properties | consumer_tag | reply_text | class_id
       | no_local | method_id | exchange | routing_key
       | routing_key | version_major | requeue | exclusive
       | realm | mechanisms | exclusive | content_checksum
       | passive | locales | write | read
       | delivery_tag | active | mechanism | ticket
       | integer_3 | locale | integer_4 | consumer_tag
       | ticket | exchange | requeue | redelivered
       | type | reply_text | reply_text | passive
       | no_ack | consumer_tag | durable | immediate
       | auto_delete | internal | exchange | client_properties
       | nowait | requeue | arguments | integer_1
       | string_2 | exchange | ticket | exchange
       | ticket | if_unused | routing_key | prefetch_size
       | nowait | exchange | message_count | consumer_tag
       | routing_key | table | integer_op | consumer_tag
       | ticket | response | queue | redelivered
       | dtx_identifier | passive | durable | exclusive
       | exchange | auto_delete | no_local | exchange
       | nowait | consumer_tag | arguments | ticket
       | prefetch_size | requeue | queue | queue
       | message_count | queue | consumer_count | string_1
       | identifier | consumer_tag | result | delivery_tag
       | exclusive | nowait | ticket | queue
       | exchange | routing_key | nowait | staged_size
       | routing_key | arguments | challenge | exchange
       | consumer_tag | queue | nowait | operation
       | immediate | routing_key | ticket | prefetch_size
       | redelivered | queue | result | nowait
       | nowait | ticket | immediate | exchange
       | meta_data | message_count | mandatory | consumer_tag
       | prefetch_count | mandatory | response | ticket
       | exclusive | queue | integer_2 | if_unused
       | consume_rate | if_empty | delivery_tag | nowait
       | delivery_tag | routing_key | prefetch_count | integer_result
       | global | ticket | message_count | consumer_tag
       | queue | delivery_tag )).

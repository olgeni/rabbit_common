...
-type(amqp_method_field_name() ::
       ( staged_size | routing_key | ticket | mechanism
       | queue | response | locale | prefetch_count
       | prefetch_count | string_1 | routing_key | challenge
       | message_count | result | if_unused | response
       | consumer_tag | mandatory | consumer_tag | delivery_tag
       | nowait | identifier | exchange | channel_max
       | prefetch_size | integer_op | queue | heartbeat
       | routing_key | if_empty | consumer_tag | no_local
       | channel_max | frame_max | consumer_tag | heartbeat
       | consume_rate | nowait | virtual_host | immediate
       | capabilities | exchange | nowait | insist
       | nowait | ticket | content_size | known_hosts
       | prefetch_count | host | known_hosts | delivery_tag
       | immediate | global | reply_code | reply_text
       | class_id | exclusive | method_id | nowait
       | consumer_tag | prefetch_size | delivery_tag | no_local
       | no_ack | routing_key | out_of_band | mandatory
       | nowait | consumer_tag | active | redelivered
       | consumer_tag | consumer_tag | integer_result | active
       | exchange | cluster_id | routing_key | reply_code
       | consumer_tag | reply_text | integer_2 | details
       | delivery_tag | exchange | identifier | reply_code
       | delivery_tag | reply_text | multiple | reply_code
       | class_id | string_op | method_id | exchange
       | routing_key | consumer_tag | routing_key | operation
       | mandatory | realm | exclusive | string_result
       | passive | active | queue | ticket
       | ticket | write | ticket | read
       | routing_key | redelivered | ticket | integer_1
       | frame_max | exchange | consumer_tag | queue
       | ticket | queue | exchange | exchange
       | type | passive | durable | delivery_tag
       | reply_code | auto_delete | routing_key | internal
       | nowait | global | global | arguments
       | string_2 | multiple | operation | queue
       | ticket | ticket | exchange | integer_3
       | if_unused | nowait | message_count | consumer_tag
       | requeue | table | ticket | arguments
       | reply_text | immediate | ticket | no_ack
       | queue | passive | durable | exclusive
       | no_ack | server_properties | auto_delete | exclusive
       | content_checksum | nowait | dtx_identifier | consumer_tag
       | arguments | exchange | requeue | queue
       | message_count | requeue | consumer_count | prefetch_size
       | delivery_tag | exchange | no_local | exclusive
       | routing_key | reply_text | ticket | queue
       | exchange | delivery_tag | reply_code | routing_key
       | nowait | arguments | consumer_tag | nowait
       | integer_4 | ticket | redelivered | reply_text
       | ticket | queue | nowait | version_major
       | identifier | result | version_minor | exchange
       | client_properties | message_count | requeue | meta_data
       | mechanisms | locales | ticket | consumer_tag
       | queue | exchange )).

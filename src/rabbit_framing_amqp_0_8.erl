...
-type(amqp_method_field_name() ::
       ( prefetch_size | exchange | locales | queue
       | ticket | routing_key | client_properties | mandatory
       | mechanism | nowait | response | ticket
       | locale | redelivered | reply_code | consumer_tag
       | exclusive | challenge | message_count | response
       | prefetch_size | routing_key | consumer_tag | channel_max
       | reply_text | requeue | queue | integer_4
       | heartbeat | routing_key | channel_max | consumer_tag
       | frame_max | content_size | heartbeat | nowait
       | consume_rate | queue | delivery_tag | ticket
       | virtual_host | immediate | capabilities | integer_1
       | insist | consumer_tag | message_count | known_hosts
       | if_unused | cluster_id | prefetch_count | consumer_tag
       | host | integer_op | identifier | known_hosts
       | operation | prefetch_count | result | reply_code
       | mandatory | reply_text | prefetch_size | if_empty
       | class_id | consumer_tag | nowait | method_id
       | integer_result | consumer_tag | no_local | routing_key
       | no_ack | out_of_band | nowait | nowait
       | nowait | no_local | integer_2 | active
       | consumer_tag | operation | delivery_tag | active
       | table | exchange | global | prefetch_count
       | reply_code | immediate | reply_text | details
       | global | identifier | consumer_tag | routing_key
       | delivery_tag | reply_code | reply_text | delivery_tag
       | consumer_tag | class_id | method_id | meta_data
       | exchange | routing_key | nowait | mandatory
       | realm | queue | redelivered | exclusive
       | consumer_tag | delivery_tag | passive | active
       | write | string_result | read | exchange
       | string_2 | identifier | delivery_tag | ticket
       | string_op | exchange | routing_key | global
       | ticket | no_ack | exchange | multiple
       | type | reply_code | passive | durable
       | reply_code | auto_delete | internal | nowait
       | ticket | ticket | arguments | ticket
       | requeue | ticket | exchange | consumer_tag
       | if_unused | queue | frame_max | exchange
       | integer_3 | exchange | ticket | exchange
       | queue | exchange | consumer_tag | routing_key
       | passive | delivery_tag | durable | exclusive
       | routing_key | auto_delete | nowait | redelivered
       | arguments | requeue | exchange | queue
       | ticket | result | queue | message_count
       | consumer_count | string_1 | arguments | immediate
       | ticket | ticket | consumer_tag | queue
       | no_local | exchange | content_checksum | ticket
       | routing_key | multiple | nowait | routing_key
       | arguments | exclusive | no_ack | reply_text
       | server_properties | nowait | staged_size | exchange
       | dtx_identifier | ticket | queue | exclusive
       | delivery_tag | nowait | requeue | consumer_tag
       | version_major | version_minor | queue | message_count
       | mechanisms | reply_text )).

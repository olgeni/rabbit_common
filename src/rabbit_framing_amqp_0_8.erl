...
-type(amqp_method_field_name() ::
       ( string_result | delivery_tag | ticket | ticket
       | queue | nowait | string_1 | passive
       | reply_text | durable | exclusive | auto_delete
       | immediate | global | nowait | challenge
       | arguments | no_local | queue | reply_text
       | message_count | consumer_count | prefetch_size | redelivered
       | consumer_tag | operation | ticket | queue
       | integer_4 | exchange | consumer_tag | routing_key
       | nowait | ticket | nowait | type
       | arguments | consumer_tag | exchange | exchange
       | delivery_tag | routing_key | delivery_tag | ticket
       | queue | ticket | nowait | mandatory
       | immediate | string_2 | message_count | reply_code
       | multiple | channel_max | consumer_tag | reply_code
       | ticket | exchange | queue | delivery_tag
       | if_unused | consumer_tag | result | if_empty
       | ticket | frame_max | mandatory | nowait
       | consumer_tag | dtx_identifier | identifier | delivery_tag
       | message_count | exchange | nowait | routing_key
       | exchange | queue | ticket | heartbeat
       | consumer_tag | integer_1 | queue | consumer_tag
       | exchange | routing_key | arguments | consumer_tag
       | consumer_tag | queue | routing_key | routing_key
       | delivery_tag | consumer_tag | routing_key | redelivered
       | meta_data | exchange | queue | queue
       | routing_key | message_count | ticket | prefetch_size
       | arguments | prefetch_count | mandatory | host
       | nowait | known_hosts | consumer_tag | prefetch_count
       | no_ack | no_ack | reply_code | realm
       | consumer_tag | reply_text | operation | class_id
       | routing_key | method_id | delivery_tag | content_size
       | requeue | consume_rate | exclusive | exclusive
       | exclusive | out_of_band | requeue | delivery_tag
       | no_local | integer_2 | active | queue
       | requeue | nowait | nowait | active
       | passive | exchange | string_op | reply_code
       | reply_text | ticket | details | identifier
       | active | reply_code | cluster_id | virtual_host
       | reply_text | integer_result | class_id | method_id
       | global | exchange | identifier | routing_key
       | exclusive | if_unused | version_major | reply_text
       | version_minor | no_ack | server_properties | staged_size
       | mechanisms | locales | multiple | write
       | reply_code | read | client_properties | insist
       | mechanism | response | content_checksum | locale
       | prefetch_count | no_local | ticket | ticket
       | exchange | routing_key | response | passive
       | global | durable | auto_delete | internal
       | ticket | nowait | exchange | table
       | redelivered | integer_3 | channel_max | frame_max
       | exchange | heartbeat | ticket | result
       | requeue | exchange | consumer_tag | integer_op
       | capabilities | prefetch_size | nowait | immediate
       | known_hosts | consumer_tag )).

...
-type(amqp_method_field_name() ::
       ( ticket | queue | queue | passive
       | string_2 | durable | exchange | exclusive
       | consumer_tag | auto_delete | identifier | nowait
       | consumer_tag | prefetch_size | arguments | redelivered
       | delivery_tag | exchange | queue | consumer_tag
       | message_count | integer_1 | consumer_count | staged_size
       | dtx_identifier | type | consumer_tag | cluster_id
       | ticket | consumer_tag | queue | exchange
       | operation | consumer_tag | routing_key | nowait
       | delivery_tag | arguments | routing_key | nowait
       | global | routing_key | ticket | multiple
       | mandatory | durable | queue | nowait
       | consumer_tag | routing_key | message_count | operation
       | delivery_tag | ticket | ticket | frame_max
       | ticket | consumer_tag | queue | meta_data
       | if_unused | redelivered | content_checksum | if_empty
       | delivery_tag | nowait | queue | heartbeat
       | immediate | exchange | requeue | content_size
       | message_count | consume_rate | nowait | ticket
       | routing_key | delivery_tag | queue | queue
       | delivery_tag | nowait | exchange | routing_key
       | exchange | result | arguments | nowait
       | requeue | arguments | mandatory | identifier
       | delivery_tag | mandatory | ticket | consumer_tag
       | consumer_tag | integer_result | prefetch_size | version_minor
       | no_ack | ticket | prefetch_count | no_ack
       | global | multiple | reply_code | exchange
       | immediate | known_hosts | prefetch_count | reply_code
       | exclusive | exclusive | reply_text | exclusive
       | no_local | class_id | requeue | reply_code
       | method_id | immediate | consumer_tag | reply_code
       | no_local | no_ack | global | passive
       | out_of_band | delivery_tag | nowait | redelivered
       | no_local | exchange | active | integer_3
       | consumer_tag | active | virtual_host | exchange
       | active | reply_code | reply_text | details
       | identifier | ticket | reply_code | reply_text
       | reply_text | capabilities | class_id | string_result
       | method_id | exchange | routing_key | string_1
       | version_major | requeue | nowait | realm
       | read | queue | server_properties | mechanisms
       | string_op | locales | write | client_properties
       | reply_text | routing_key | mechanism | response
       | locale | consumer_tag | exchange | challenge
       | table | exchange | integer_2 | response
       | ticket | consumer_tag | passive | channel_max
       | auto_delete | internal | nowait | prefetch_size
       | ticket | channel_max | queue | integer_op
       | exchange | integer_4 | frame_max | reply_text
       | heartbeat | routing_key | ticket | exchange
       | if_unused | nowait | ticket | host
       | insist | prefetch_count | message_count | result
       | known_hosts | routing_key | routing_key | consumer_tag
       | exclusive | ticket )).

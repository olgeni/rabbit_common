...
-type(amqp_method_field_name() ::
       ( queue | requeue | global | queue
       | message_count | consumer_count | routing_key | no_local
       | exclusive | result | mandatory | ticket
       | channel_max | consumer_tag | queue | exchange
       | immediate | routing_key | delivery_tag | nowait
       | arguments | ticket | string_2 | version_minor
       | routing_key | nowait | redelivered | ticket
       | consume_rate | queue | server_properties | exclusive
       | nowait | operation | delivery_tag | exchange
       | immediate | message_count | mandatory | content_size
       | multiple | immediate | requeue | delivery_tag
       | ticket | queue | if_unused | if_empty
       | nowait | content_checksum | requeue | prefetch_count
       | locales | string_result | global | virtual_host
       | exchange | message_count | no_ack | ticket
       | consumer_tag | reply_code | no_local | ticket
       | queue | capabilities | exchange | redelivered
       | consumer_tag | routing_key | arguments | string_1
       | no_ack | exclusive | reply_text | nowait
       | client_properties | delivery_tag | requeue | insist
       | consumer_tag | reply_text | prefetch_size | consumer_tag
       | prefetch_count | ticket | message_count | global
       | consumer_tag | consumer_tag | consumer_tag | integer_4
       | ticket | response | ticket | queue
       | table | staged_size | consumer_tag | reply_code
       | no_local | reply_text | queue | no_ack
       | exchange | identifier | out_of_band | consumer_tag
       | consumer_tag | nowait | exchange | active
       | consumer_tag | active | exchange | exchange
       | delivery_tag | reply_code | host | passive
       | reply_text | integer_3 | details | result
       | identifier | reply_code | reply_text | routing_key
       | known_hosts | class_id | ticket | method_id
       | exchange | consumer_tag | prefetch_size | dtx_identifier
       | routing_key | version_major | realm | redelivered
       | exclusive | exchange | prefetch_count | mechanisms
       | active | write | read | mechanism
       | consumer_tag | durable | ticket | type
       | integer_result | locale | delivery_tag | ticket
       | consumer_tag | operation | challenge | queue
       | meta_data | exchange | response | routing_key
       | passive | reply_text | channel_max | frame_max
       | routing_key | heartbeat | nowait | queue
       | nowait | arguments | reply_code | class_id
       | frame_max | durable | routing_key | heartbeat
       | ticket | routing_key | exchange | integer_op
       | if_unused | nowait | nowait | identifier
       | auto_delete | ticket | method_id | known_hosts
       | exchange | cluster_id | mandatory | routing_key
       | ticket | queue | nowait | integer_2
       | passive | exchange | delivery_tag | reply_code
       | internal | integer_1 | exclusive | delivery_tag
       | prefetch_size | auto_delete | multiple | nowait
       | string_op | arguments )).

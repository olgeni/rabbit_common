...
-type(amqp_method_field_name() ::
       ( exchange | requeue | passive | routing_key
       | active | write | read | reply_code
       | ticket | nowait | delivery_tag | exchange
       | immediate | consumer_tag | ticket | exchange
       | operation | content_checksum | ticket | type
       | passive | reply_code | exclusive | durable
       | auto_delete | reply_text | internal | nowait
       | arguments | dtx_identifier | exchange | reply_text
       | requeue | ticket | consumer_tag | exchange
       | if_unused | staged_size | nowait | queue
       | ticket | routing_key | integer_4 | queue
       | ticket | routing_key | queue | passive
       | queue | meta_data | durable | multiple
       | mechanism | exclusive | exchange | prefetch_size
       | auto_delete | result | exchange | nowait
       | table | arguments | global | mandatory
       | delivery_tag | nowait | queue | if_empty
       | consumer_tag | consumer_tag | message_count | ticket
       | consumer_count | string_2 | ticket | identifier
       | queue | consumer_tag | content_size | nowait
       | exchange | nowait | consume_rate | routing_key
       | queue | consumer_tag | exchange | nowait
       | arguments | prefetch_count | immediate | global
       | prefetch_size | exclusive | ticket | integer_op
       | string_1 | queue | nowait | consumer_tag
       | prefetch_size | delivery_tag | consumer_tag | message_count
       | cluster_id | consumer_tag | integer_result | mechanisms
       | locales | routing_key | operation | ticket
       | redelivered | consumer_tag | client_properties | if_unused
       | integer_1 | response | nowait | locale
       | consumer_tag | delivery_tag | challenge | delivery_tag
       | nowait | message_count | response | routing_key
       | channel_max | queue | consumer_tag | heartbeat
       | delivery_tag | reply_code | routing_key | multiple
       | string_op | channel_max | routing_key | frame_max
       | heartbeat | delivery_tag | virtual_host | routing_key
       | ticket | capabilities | ticket | integer_2
       | insist | delivery_tag | message_count | ticket
       | known_hosts | prefetch_count | consumer_tag | ticket
       | host | frame_max | exchange | known_hosts
       | queue | prefetch_count | reply_code | reply_text
       | class_id | result | method_id | arguments
       | consumer_tag | exchange | string_result | routing_key
       | global | no_local | identifier | no_ack
       | immediate | out_of_band | nowait | mandatory
       | no_local | queue | mandatory | active
       | requeue | no_ack | consumer_tag | active
       | exchange | redelivered | reply_code | reply_text
       | reply_text | ticket | details | version_minor
       | redelivered | identifier | no_local | reply_code
       | requeue | reply_text | class_id | method_id
       | no_ack | server_properties | exchange | exclusive
       | routing_key | exchange | version_major | realm
       | exclusive | integer_3 )).
